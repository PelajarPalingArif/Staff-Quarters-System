            ...
            <div class="fw-bold mt-4 w-100 ms-3 mb-4" style="font-family: 'Open Sans', sans-serif;font-size:40px;color:black;">
                ...
                if (isset($type)) {
                    if ($type == "1") {
                        ...
                        echo "$_GET[empID]'s Rooms";
                    }
                } else if (!isset($unitID)) {
                    echo "All's Rooms";
            ...
            <div class="row mt-4 pt-4 pb-4" style="background-color:rgb(0, 48, 0);color:white">
                <table id="buildTable" class="table table-success  table-hover border border-success table-bordered mt-4">
                    ...
                    <tbody class="table-group-divider">
                        ...
                        if (isset($type)) {
                            if ($type == "1") {
                                $listUnitsQuery = "SELECT r.`UnitID`,u.`UnitCode`,r.`RoomID`,r.`Status`,COALESCE(e.`EmployeeID`, 'None') as 'EmployeeID',COALESCE(e.`Name`,'None') as 'Name' FROM `rooms` r left JOIN `employee` e on e.`EmployeeID` = r.`EmployeeID` left JOIN `unit` u on r.UnitID = u.UnitID WHERE r.Status = '0'";
                            } else if ($type == "0") {
                                $listUnitsQuery = "SELECT r.`UnitID`,u.`UnitCode`,r.`RoomID`,r.`Status`,COALESCE(e.`EmployeeID`, 'None') as 'EmployeeID',COALESCE(e.`Name`,'None') as 'Name' FROM `rooms` r left JOIN `employee` e on e.`EmployeeID` = r.`EmployeeID` left JOIN `unit` u on r.UnitID = u.UnitID WHERE r.Status = '1'";
                            }
                            // For Individual Employee
                            else {
                                $empID1 = $_GET['empID'];
                                $listUnitsQuery = "SELECT r.`UnitID`,u.`UnitCode`,r.`RoomID`,r.`Status`,COALESCE(e.`EmployeeID`, 'None') as 'EmployeeID',COALESCE(e.`Name`,'None') as 'Name' FROM `rooms` r left JOIN `employee` e on e.`EmployeeID` = r.`EmployeeID` left JOIN `unit` u on r.UnitID = u.UnitID WHERE e.`EmployeeID` = '$empID1'";
                            }
                        } else if (!isset($unitID)) {
